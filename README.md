...
PulseField™ enables independent parallel sequences for efficient PRNGs (such as [BlastCircuit™](https://github.com/ghostproxies/blastcircuit) and [RecoilFuse™](https://github.com/ghostproxies/recoilfuse)).
...
[pulsefield.c](pulsefield.c) is an exhaustive PulseField™ validation (when `BITS_LENGTH` is `8` and the `stdint.h` header defines an 8-bit, unsigned integral type for `uint8_t`).
